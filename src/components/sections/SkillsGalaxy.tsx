      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
          {categories.map((category, index) => (
            <Magnetic key={category} strength={0.06} className="h-full">
              <motion.div
              ...
              >
                <div className="relative bg-black pixel-border p-5 md:p-6 h-full flex flex-col hover:bg-cyan-950 pixel-shadow-hover transition-none">
                  <h3 className="text-base md:text-lg font-display text-white uppercase mb-4 md:mb-6 flex items-center gap-3 md:gap-4">
                    <span className="w-2 h-2 md:w-3 md:h-3 bg-cyan-400 border-2 border-white" />
                    {category}
                  </h3>
